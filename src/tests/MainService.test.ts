...
describe("MainService", () => {
  ...
  beforeEach(() => {
    ...
    fetchComplianceHistoryMock = sinon.stub(
      NotificationService.prototype,
      "fetchRelevantComplianceHistory",
    );
  ...
  it("should call fetch compliance history and send to slack", async () => {
    ...
    getInputMock
      .withArgs("slackAccessToken", sinon.match.any)
      .returns("dummyToken");
    ...
    expect(
      sendSlackMessageMock.calledOnceWith(
        historyCompliance,
        sinon.match.any,
        sinon.match.any,
      ),
  ...
  it("should return false when channel name is missing", async () => {
    ...
    await expect(new MainService().run()).to.be.rejectedWith(
      "Missing slack channel name in config",
    );

...
export class SlackService {
  ...
  public async sendToSlack(
    ...
    const requestBody: SlackPostMessagePayload = createSlackMessage(
      channel,
      complianceHistory,
      dashboardUrl,
    );

    const response: SlackResponse = await sendMessage(
      requestBody,
    ...
    if (!response.ok) {
      throw new Error(
        "Error when posting slack message. With error message from slack : " +
          response.error,
      );
    }
  }
...
async function sendMessage(
  request: SlackPostMessagePayload,
  accessToken: string,
